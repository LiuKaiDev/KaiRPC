...
// 返回可写的字节数
    ...
    void TcpBuffer::moveReadIndex(int size) {
        ...
        if (size < 0 || j > m_write_index) {
            ERRORLOG("moveReadIndex error, invalid size %d, old_read_index %d, write_index %d", size, m_read_index, m_write_index);
            return;
        }
        m_read_index = j;
        adjustBuffer();
