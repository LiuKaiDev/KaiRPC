//
// Created for KaiRPC on 23-10-12.
//
...
constexpr int MIN_HEARTBEAT_INTERVAL_MS = 100;

int getServiceTTL() {
    auto config = Config::getServiceCenterMap();
    auto it = config.find("service_ttl_ms");
    if (it == config.end() || it->second.empty()) {
        return DEFAULT_SERVICE_TTL_MS;
    }

    int ttl_ms = std::atoi(it->second.c_str());
    return ttl_ms > 0 ? ttl_ms : DEFAULT_SERVICE_TTL_MS;
}

void startServiceHeartbeat(const std::vector<std::string>& service_names,
                           const std::string& remote_ip,
                           int control_port,
                           const std::string& local_ip,
                           int local_port) {
    int ttl_ms = getServiceTTL();
    int interval_ms = std::max(MIN_HEARTBEAT_INTERVAL_MS, ttl_ms / 2);

    std::thread([service_names, remote_ip, control_port, local_ip, local_port,
                 interval_ms]() {
        while (true) {
            ...
            for (const auto& service_name : service_names) {
                auto res =
                    command.heartbeat_service(service_name, local_ip, local_port);
                if (!res.get_is_success()) {
                    ERRORLOG("heartbeat service discovery failed [%s]: [%s]",
                             service_name.c_str(),
                             res.get_info().c_str());
                }
            }
...
#define DELETE_RESOURCE(XX) \
  if (XX != NULL) { \
    ...
    void RpcDispatcher::registerService(const service_s_ptr &service) {
        ...
        // remote  registe
        std::string remote_ip = talon::Config::getServiceCenterMap()["service_center_ip"];
        ...
        for (int i = 0; i < service->GetDescriptor()->method_count(); ++i) {
            ...
            registered_services.push_back(all_name);
        }

        if (!registered_services.empty()) {
            startServiceHeartbeat(registered_services, remote_ip, control_port, local_ip, local_port);
        }
