...
if (!name##_node) { \
  printf("Start KaiRPC server error, failed to read node [%s]\n", #name); \
  exit(0); \
} \
...
#define READ_STR_FROM_XML_NODE(name, parent) \
  ...
  if (!name##_node|| !name##_node->GetText()) { \
    printf("Start KaiRPC server error, failed to read config file %s\n", #name); \
    exit(0); \
  } \
...
namespace talon {
    ...
    Config::Config(const char *xmlfile) {
        ...
        if (!rt) {
            printf("Start KaiRPC server error, failed to read config file %s, error info[%s] \n", xmlfile,
    ...
    void Config::setServiceCenterMap(const std::string &conf_path) {
        ...
        if (service_center_map.find("service_ttl_ms") == service_center_map.end() ||
            service_center_map["service_ttl_ms"].empty()) {
            service_center_map["service_ttl_ms"] = "10000";
        }
    }
